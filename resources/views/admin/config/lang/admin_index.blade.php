    ...
    <x-admin.hmtl.section>
        <div class="content mb-3">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-9">
                        <x-admin.form.select-arr   :send-arr="config('adminLangFile.adminFile')" name="selectfile" :sendvalue="$selId" label="{{__('admin/config/core.lang_select_file')}}" :labelview="false" />
                    </div>
                    <div class="col-3 dir_button">
                        <x-admin.form.action-button url="{{route('adminlang.edit')}}"  print-lable="{{__('admin/config/core.lang_add_new_key')}}" size="m" :tip="false" bg="dark" />
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <table  id="MainDataTable" class="table table-bordered table-hover"  >
                        ...
                        @foreach($rowData as $row)
                            <tr>
                                ...
                                @endforeach
                                <td class="TD_20">
                                    <a href="#" thisid="custmid_{{$loop->index}}" class="btn btn-sm btn-primary copyThisText"><i class="fa fas fa-copy"></i></a>
                                    <input value="__('{{$row['prefixCopy']}}')" id="custmid_{{$loop->index}}" type="hidden">
                                </td>

                                <td class="TD_20">
                                    <a href="#" thisid="Newcustmid_{{$loop->index}}" class="btn btn-sm btn-dark copyThisText"><i class="fa fas fa-copy"></i></a>
