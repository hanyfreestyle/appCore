<div class="{{$colrow}}">
    <div class="form-group">
        ...
        <select class="form-control select2 custom-select @error($name) is-invalid @enderror " id="{{$name}}" name="{{$name}}" style="width: 100%;" >
            ...
            @elseif($selectType == 'changeLang')
                @foreach ($sendArr as  $key => $value)
                    <option value="{{ $value['id'] }}" @if ($value['id'] == $sendvalue) selected @endif>{!! $value->translate($applang)->name ?? $value->translate($changelang)->name ?? ''!!}</option>
                @endforeach
